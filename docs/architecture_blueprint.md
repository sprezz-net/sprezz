...
2. **`FEP-4ccd` (Pending Followers Collection and Pending Following Collection)**: Standardizes managing pending follow requests using dedicated collections. Partially implemented via our `"pending_follows"` collection and manual state machine (`AcceptFollow` and `RejectFollow`) writing state quads on follow activity subjects.
3. **`FEP-8c13` (Context-Authority Routing with Object Integrity Proofs for Restricted Threads)**: Standardizes routing and boundaries for restricted threads. Aligned in Sprezz by enforcing context-aware audience target pruning (for direct or followers-only visibility) to prevent private notes from leaking into unauthorized collection responses.
