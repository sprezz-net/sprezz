...
4. **Object Integrity Proof Precedence**: In any incoming ActivityPub request where the body payload contains a valid `DataIntegrityProof` with the `eddsa-jcs-2022` cryptosuite (FEP-8b32) or a FEP-8c13 Author/Forwarding Proof, the verifier validates the integrity proof first. This processing is performed via modular, low-complexity helper routines to verify the Author, Forwarding, or Object Integrity Proof. If verification is successful, standard HTTP Signature verification is bypassed and the actor's identity is asserted.
...
7. **JSON-LD Structure Depth Guard (JSON Bomb Prevention)**: To defend against JSON bomb amplification attacks trying to exploit memory usage before or during unmarshaling, all incoming ActivityPub payloads are validated using a streaming JSON tokenizer with $O(1)$ memory consumption. Payloads nested past 100 levels are early-rejected with `HTTP 400 Bad Request`. This depth limit is enforced twice as a defense-in-depth practice: once in the `SignatureValidator` middleware to protect the verifier's own internal `json.Unmarshal` step during Author Proof/Integrity Proof checks, and once again at the `GenericHandler` endpoint perimeter.
8. **Ingestion TOCTOU Defense (Atomic Queue Persistence)**: To prevent HTTP Signature Time-of-Check to Time-of-Use (TOCTOU) replay attacks where a client context cancellation cancels the database transaction after signature verification, the ingestion queue write (`EnqueueInbound`) runs on a value-preserving detached context wrapped with a secure, dedicated 5-second database timeout. This guarantees that once a request passes validation, its unique `activity_iri` commits atomically, making replaying the exact activity payload impossible due to database uniqueness constraints.
9. **Asynchronous Bounded Task Execution (Context Leakage Prevention)**: Background tasks that continue executing after the HTTP response has returned (such as Followers synchronization or context backfills) are completely decoupled from the parent HTTP request context (which is cancelled immediately upon request termination). Instead, they run on value-preserving detached contexts combined with a strict, bounded 30-second execution timeout. This guarantees task completion while successfully mitigating unbounded goroutine thread leaks.
...
2. **Chronological Key History Window Routing and One-Time Cache Lookups**: The verification system performs one-time public key fetching to optimize network overhead and database access. When a signature or integrity proof verification requires a public key, the system queries the local key history cache first (`GetHistoricalKey`) using a compound lookup to locate the key valid at the request's chronological timestamp. If a remote key is not found in the local cache, a single outbound HTTP de-referencing fetch is executed, and the resulting PEM is archived immediately. Subsequent verifications hit the local cache directly, completely bypassing redundant remote lookups. If verification fails against a cached key, the cache is invalidated, and a fresh key is fetched exactly once to handle rotated keys gracefully.
...
4. **FEP-fe34 Origin-Based Security and Ownership Compliance**: In accordance with FEP-fe34, the system recursively parses the created object's properties (such as `attributedTo`). It strictly verifies that (a) the origin domain of the object's `id` matches the origin domain of its owner/`attributedTo` identifier, and (b) the activity's signing `actor` is exactly identical to the object's `attributedTo` owner (compared after scheme/host lowercase normalization). This completely eliminates inbound object content-spoofing where a remote actor attempts to publish an activity containing an object supposedly attributed to a victim actor.
...
20. **`FEP-fe34` (Origin-based security model)**: Standardizes security boundaries using same-origin policy checks on object ownership (verifying that object `id` and `attributedTo` have matching origin domains) and creator authorization (verifying that the activity's `actor` matches the object's `attributedTo` owner). Fully implemented inside our core inbound activity validation layers.
