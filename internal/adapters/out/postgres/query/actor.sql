...
SELECT
    ...
    COALESCE(o_term.value, q.literal_value)::TEXT AS object
FROM rdf_quads q
...
LEFT JOIN rdf_dictionary o_term ON q.object_id = o_term.id
WHERE q.graph_id = (
      ...
      LEFT JOIN rdf_dictionary inner_o ON inner_q.object_id = inner_o.id
      WHERE inner_p.value = 'https://www.w3.org/ns/activitystreams#preferredUsername'
        ...
        AND COALESCE(inner_o.value, inner_q.literal_value) = @username::text
        ...
        AND EXISTS (
            ...
            LEFT JOIN rdf_dictionary type_o ON type_q.object_id = type_o.id
            WHERE type_q.graph_id = inner_q.graph_id
              ...
              AND COALESCE(type_o.value, type_q.literal_value) IN (
...
WHERE p_term.value = 'https://www.w3.org/ns/activitystreams#alsoKnownAs'
  AND COALESCE(o_term.value, q.literal_value) = $1
LIMIT 1;
